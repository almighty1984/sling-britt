...
namespace history {
    ...
    const std::string_view to_string(const Act& act) {
        switch (act) {
    ...
    struct Info {
        ...
        bool operator ==(const Info& other) {
            return (layer == other.layer and tile_set == other.tile_set and rect == other.rect and offset == other.offset);
        }
        bool operator !=(const Info& other) {
...
namespace menu {
    ...
    struct List {
        ...
        ~List() {
            transform::erase(transform);
            sprite::erase(bg_sprite);
            text_items.clear();
        }

        void draw(std::unique_ptr<Window>& window) {
            sprite::draw(window, bg_sprite);
...
export namespace sheet {
    ...
    private:
        I32 m_mouse_transform           = -1,
            m_level_transform           = -1,
            m_tile_set_transform        = -1,
            m_tile_set_bg_transform     = -1,
            m_grid_transform            = -1, m_grid_map_transform   = -1,
            m_menu_up_transform         = -1,
            m_menu_down_transform       = -1, m_menu_right_transform = -1;

        I32 m_menu_down_bg_sprite             = -1, m_menu_right_bg_sprite = -1,
            m_position_on_grid_map_sprite     = -1,
            m_save_sprite                     = -1,
            m_tile_set_sprite                 = -1, m_tile_set_bg_sprite   = -1,
            m_current_tile_sprite             = -1,
            m_grid_icon_sprite                = -1,
            m_active_layer_sprite             = -1,
            m_text_bar_bg_sprite              = -1,
            m_text_current_tile_set_bg_sprite = -1;

        std::vector<I32> m_is_hidden_layer_sprites,
                         m_mouse_sprites,
                         m_selection_on_tile_set_sprites,
                         m_selection_on_level_sprites,
                         m_moving_sprites;

        std::list<I32> m_level_sprites;
        ...
        cI32 grid_sprite_at_offset(cVec2F offset);
        cI32 grid_map_sprite_at_offset(cVec2F offset);
        ...
        bool load_types_from_text_file(const std::filesystem::path& path);
        ...
        cI32 level_sprite_at_offset(cU8 layer, cVec2F offset);

        history::Act place_sprite_on_level(cU8 layer, cU8 tile_set, cRectI rect, cVec2F offset);
