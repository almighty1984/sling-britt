    ...
    void Brick::collide_x(aabb::cInfo our, aabb::cInfo other) {
        ...
        switch (other_type) {
            ...
            case Type::brick: {
                ...
                if (our_velocity.x < 0.0F and other.owner->is_near_wall_L()) {
                    m_is_near_wall_L = true;
                    velocity_x(0.0F);
                    move_velocity_x(0.0F);
                    return;
                }
                if (our_velocity.x > 0.0F and other.owner->is_near_wall_R()) {
                    m_is_near_wall_R = true;
                    velocity_x(0.0F);
                    move_velocity_x(0.0F);
                    return;
            ...
            case Type::bug: {
                if (m_state == state::Type::carried or other.owner->state() == state::Type::carried) return;
            ...
            case Type::clip_LD:
            case Type::clip_RD: {
                if ((other_type == Type::clip_L or other_type == Type::clip_L_50 or other_type == Type::clip_LD) and our_velocity.x < 0.0F or
                    (other_type == Type::clip_R or other_type == Type::clip_R_50 or other_type == Type::clip_RD) and our_velocity.x > 0.0F) {
                    return;
                }
            ...
            case Type::player: {
                if (other_velocity.y >= 6.0F) {
                    //console::log(class_name(), "::collide_x() player\n");
                    hurt(other.owner);
                    return;
                }
                if ((other_velocity.x < 0.0F and m_is_near_wall_L) or
                ...
                if ((other_velocity.x < 0.0F and other_UL.x < our_UL.x) or
                    (other_velocity.x > 0.0F and other_DR.x > our_DR.x)) {
                    return;
                }
