...
namespace entity {
    void Bug::collide_x(aabb::cInfo our, aabb::cInfo other) {
        ...
        //    (other_type == Type::brick and !other.owner->is_tossed() and other.owner->state() != state::Type::dead)) {
        ...
        if (m_state == state::Type::carried) {
            if (other_type == Type::spring_U) return;
        ...
        if (m_state == state::Type::swim) {
            switch (other_type) {
                ...
                case Type::clip_ledge_R_50: {
                    ...
                    if (other_type == Type::clip_L or
                        other_type == Type::clip_L_50 or
                        other_type == Type::clip_ledge_L_50 or
                        (other_type == Type::clip_ledge and our_velocity.x > 0.0F)) {
                        if (our_UL.x < other_UL.x) {
                            if (velocity().x > -1.0F) {
                                velocity_x(-1.0F);
                            }
                        }
                    } else if (other_type == Type::clip_R or
        ...
        if (m_state == state::Type::tossed) {
            if (our_DR.y < other_UL.y + 2.0F or our_UL.y > other_DR.y - 2.0F) return;

            if (is_arch(other_type) or
                other_type == Type::brick or
                other_type == Type::clip  or
                ((other_type == Type::clip_L or other_type == Type::clip_LD or other_type == Type::clip_ledge_L_50) and our_velocity.x > 0.0F) or
                ((other_type == Type::clip_R or other_type == Type::clip_RD or other_type == Type::clip_ledge_R_50) and our_velocity.x < 0.0F) or
        ...
        switch (other_type) {
            ...
            case Type::brick: {
                ...
                if (m_state == state::Type::walk) {
                    ...
                    velocity_x(our_velocity.x * -0.9F + other_velocity.x);
                    move_velocity_x(0.0F);
                    sprite_is_leftward(!sprite_is_leftward());
            ...
            case Type::clip_ledge_L_50:
            case Type::clip_ledge_R_50: {
                if ((our_velocity.x < 0.0F and other_type == Type::clip_ledge_L_50) or
                    (our_velocity.x > 0.0F and other_type == Type::clip_ledge_R_50)) {
                    return;
                }
            ...
            case Type::player: {
                ...
                if ((other_velocity.x < 0.0F and m_is_near_wall_L) or
                    (other_velocity.x > 0.0F and m_is_near_wall_R)) {
                    other.owner->velocity_x(-other_velocity.x);
                    other.owner->velocity_y(-2.0F);
                    return;
                }
                ...
                if (m_state == state::Type::upended or m_state == state::Type::bounce) {
                    ...
                    if ((vel_sum.x <= 0.1F and !sprite_is_leftward()) or other.owner->is_near_wall_R()) {
                        sprite_is_leftward(true);
                        velocity_x(-1.0F);
                        velocity_y(-1.0F);
                        other.owner->velocity_x(velocity().x * -0.9F);
                    }
                    else if ((vel_sum.x >= -0.1F and sprite_is_leftward()) or other.owner->is_near_wall_L()) {
