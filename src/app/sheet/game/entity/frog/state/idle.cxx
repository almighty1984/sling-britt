...
namespace entity {
    void Frog::state_idle(cF32 dt) {
        if (m_is_first_state_update) {
            ...
            m_time_left_to_spawn_sense = m_config.time_to_spawn_sense() + 0;
        ...
        if (m_time_left_to_react > 0) {
            ...
            console::log(class_name(), "::state_idle() ", (int)m_time_left_to_react, " sensed position : ", m_sensed_offset.x, " ", m_sensed_offset.y, "\n");
        ...
        if (m_is_on_ground and m_sensed_state == state::Type::none) {
            for (auto& i : m_sensed) {
                ...
                switch (i->type()) {
                    case Type::mole: {
                        if (is_enemy(i)) {
                            console::log(class_name(), "::state_idle() sensed mole enemy\n");
                    ...
                    case Type::particle_health: {
                        if (health_is_max()) continue;
                        
                        console::log(class_name(), "::state_idle() sensed health\n");
                        m_sensed_state = state::Type::melee;
                        m_sensed_offset = i->position() - (position() + Vec2F{ 6.0F, 6.0F });
                        m_time_left_to_react = 1;
                    ...
                    case Type::player: {
                        if ((i->state() == state::Type::swim and i->num_jumps() == 0)) continue;
                        ...
                        if (i->state() == state::Type::duck and (i->velocity().x < -1.5F or i->velocity().x > 1.5F)) {
                            ...
                            m_sensed_offset = (i->position() + Vec2F{ 0.0F, -32.0F }) - (position() + Vec2F{ 8.0F, 8.0F });
                        } else {
