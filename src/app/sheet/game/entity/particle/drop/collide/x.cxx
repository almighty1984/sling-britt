    ...
    void ParticleDrop::collide_x(aabb::cInfo our, aabb::cInfo other) {
        ...
        switch (other_type) {
            ...
            case Type::clip_LR: {
                ...
                if (our_velocity.x < 0.0F and (other_type == Type::clip_L or other_type == Type::clip_L_50 or other_type == Type::clip_ledge_L_50) or
                    our_velocity.x > 0.0F and (other_type == Type::clip_R or other_type == Type::clip_R_50 or other_type == Type::clip_ledge_R_50) or
                ...
                if (other_type == Type::clip_ledge) {
                    if (m_is_near_wall_L) {
                        position_x(other.owner->start_offset().x + 16.0F - camera::position.x);
                    } else if (m_is_near_wall_R) {
                        position_x(other.owner->start_offset().x - camera::position.x - 4.0F);
                    }
                    m_wall_drop_on_level_y = other.owner->start_offset().y + 16.0F;
                }
                else if (other_type == Type::clip_ledge_L_50) {
                    position_x(other.owner->start_offset().x - camera::position.x + 8.0F - 4.0F);                    
                    m_wall_drop_on_level_y = other.owner->start_offset().y + 16.0F;
                } else if (other_type == Type::clip_ledge_R_50) {                    
                    position_x(other.owner->start_offset().x + 8.0F - camera::position.x);
                    m_wall_drop_on_level_y = other.owner->start_offset().y + 16.0F;
                }
            ...
            case Type::particle_drop: {
                if (m_state == state::Type::wall_slide and other_state == state::Type::wall_slide) {
                    //collide_y(our, other);
