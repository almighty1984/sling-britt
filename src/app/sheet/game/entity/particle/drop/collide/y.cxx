...
namespace entity {
    void ParticleDrop::collide_y(aabb::cInfo our, aabb::cInfo other) {
        if (other.owner == m_parent) return;
        ...
        /*if (m_state == state::Type::wall_slide) {
            collide_x(our, other);
            return;
        }*/

        cVec2F other_pos_on_level =
               is_level_geometry(other_type) ? other.owner->start_offset() : other_UL + camera::position;
        ...
        switch (other_type) {
            ...
            case Type::slope_U: {
                ...
                /*if (other_type == Type::brick and other_state != state::Type::wall_slide and 
            ...
            //console::log(m_on_level_wall_x, " ", other_UL.x + camera::position.x, "\n");
                ...
                if (our_velocity.y < 0.0F and
                    ...
                    other_type != Type::clip_U_50 and
                    other_type != Type::clip_L    and
                    other_type != Type::clip_L_50 and
                    other_type != Type::clip_R    and
                    other_type != Type::clip_R_50 and
                    ...
                    //console::log("time left dead: ", m_time_left_dead, "\n");
                           ...
                           other_type != Type::clip_D  and
                           other_type != Type::clip_D_50) {
                    ...
                    if (is_level_geometry(other_type)) {
                        if (other_type != Type::clip_L and other_type != Type::clip_L_50 and
                            other_type != Type::clip_R and other_type != Type::clip_R_50 and
                            other_type != Type::clip_LR) {
                            m_next_state = state::Type::dead;
                        }
                    ...
                    if (m_state == state::Type::wall_slide) {
                        ...
                        if (our_velocity.x == 0.0F) {
                            if (other_type == Type::clip_L or other_type == Type::clip_L_50 or
                                other_type == Type::clip_R or other_type == Type::clip_R_50 or
                                other_type == Type::clip_LR) {
                                return;
                            }
                        }
            ...
            case Type::train_saw: {
                if (m_state != state::Type::idle) return;

                m_parent = other.owner;
                m_parent_aabb = other.id;
                m_next_state = state::Type::sling;
                m_is_on_ground = false;
                if (m_config.weight() < 1.0F) {
                    m_time_to_be_in_state = random::number(CEILING_DROP_RANDOM_RANGE);
                }
                break;
            }
        }
