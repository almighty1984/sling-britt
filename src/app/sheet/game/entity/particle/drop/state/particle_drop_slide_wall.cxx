...
namespace entity {    
    void ParticleDrop::state_slide_wall(cF32 dt) {
        ...
        if (m_time_in_state % 10 == 0) {
            max_velocity_y((m_input_objects.size() + 1) * m_config.weight() * random::real_number(0.2, 0.3));
        }
        ...
        if (m_parent) {
            ...
            if (m_original_parent_position_on_level.x != 0.0F) {
                ...
                //console::log(class_name(), "::state_slide_wall() parent position diff x: ", parent_diff_x, "\n");                
                F32 offset_x = 0.0F;
                if (m_is_near_wall_L) {
                    offset_x = -1.0F;
                }
            ...
            if (m_parent->state() == state::Type::dead) {
                console::log(class_name(), "::state_slide_wall() brick died, go back to idle\n");
                m_parent = nullptr;
                m_next_state = state::Type::idle;
            }
            ...
            if (velocity().y > 0.0F and position_on_level().y > m_wall_drop_on_level_y and m_time_in_state > 5) {
                console::log(class_name(), "::state_slide_wall() drop y: ", m_wall_drop_on_level_y, "\n");
                m_parent = nullptr;
                //velocity_y(0.0F);
                m_next_state = state::Type::idle;
            }
