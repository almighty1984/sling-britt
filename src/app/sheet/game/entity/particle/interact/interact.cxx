...
namespace entity {
    void ParticleInteract::collide_x(aabb::cInfo our, aabb::cInfo other) {
        ...
        cI32 parent_aabb = m_parent->aabb(aabb::Name::body);
        cVec2F parent_DR = aabb::DR(parent_aabb);

        switch (other_type) {
            case Type::clip_ledge:
            case Type::clip_ledge_L_50: 
            case Type::clip_ledge_R_50: {
                console::log("parent_DR.y: ", parent_DR.y, " other_UL.y: ", other_UL.y, "\n");
                if (parent_DR.y <= other_UL.y) return;
                //if (our_UL.y )
