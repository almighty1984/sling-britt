...
namespace entity {
    void Player::collide_y(aabb::cInfo our, aabb::cInfo other) {
        ...
        if (our_velocity.y < 0.0F and m_state != state::Type::duck and !(m_state == state::Type::swim and !is_bouncing())) {
            if (our_UL.y > other_DR.y - 1.0F) {
        ...
        if (other.owner->state() == state::Type::swim) {
            if ((m_state == state::Type::swim or m_state == state::Type::dive) and velocity().y < 0.0F and our_DR.y > other_DR.y) {
                ...
                other.owner->velocity_y(our_velocity.y * 0.2F + other_velocity.y * 0.1F);
                if (std::abs(our_velocity.y) > 1.0F) {
                    other.owner->velocity_x(our_velocity.x * 0.75F);
                } else {
                ...
                //other.owner->move_velocity_y(our_velocity.y;
                velocity_y(0.0F);
                move_velocity_x(other_velocity.x);
        ...
        switch (other_type) {
            ...
            case Type::arch_R_2x1_1: {
                ...
                if (m_is_on_ground and !is_shoot_direction_locked_and_away) {
            ...
            case Type::brick: {
                ...
                if (our_DR.y < other_UL.y + 4.0F and our_velocity.y > 0.0F) {
                    ...
                    other.owner->weight(2.0F);
                    m_is_on_ground = true;
                    move_velocity(other_velocity);
                    velocity_y(0.0F);
                    position_add_y(-overlap_y);

                    console::log(class_name(), "::collide_y() ", other_velocity.x, " ", other_velocity.y, "\n");
                }
            ...
            case Type::bridge: {
                ...
                if (is_pressed(key_down) and is_pressed(key_jump)) {
                    ...
                    m_is_on_ground = false;
                    set_anim("fall");
                    console::log(class_name(), "::collide_y() drop from bridge\n");
                }                
                position_add_y(-overlap_y);
                velocity_y(0.0F);
                m_is_on_ground = true;
            ...
            case Type::bug: {
                ...
                if (our_UL.y + 4.0F >= other_UL.y) {   // bug above
                    Vec2F other_vel = other.owner->velocity() + other.owner->move_velocity();
                    ...
                    other.owner->velocity_x((velocity().x + move_velocity().x) + mid_diff_x / 4.0F);
            ...
            case Type::clip_UD: {
                if (our_UL.y > other_UL.y and (m_is_on_ground or m_state == state::Type::climb or m_state == state::Type::duck)) {
                    if (!is_shoot_direction_locked_and_away) {
                        m_next_state = state::Type::duck;
                        m_time_left_rising = m_config.time_to_rise();
                        return;
                    }
                ...
                if (m_state == state::Type::swim and !is_bouncing()) {                    
                    velocity_y(0.0F);
                    move_velocity_y(0.0F);
                    return;
                }
            ...
            case Type::clip_D_50: {
                if (our_UL.y - 4.0F > other_DR.y) {
                    console::log(class_name(), "::collide_y() our_UL.y - 1.0F > other_DR.y\n");
                    return;
                }
                if (our_velocity.y > 0.0F) return;
                if (m_state == state::Type::swim and our_DR.y > other_DR.y) {
                    ...
                    if (is_bouncing()) {
                        ...
                        //console::log(class_name(), "::collide_y() swim bounce\n");
                    } else {
                        velocity_y(0.0F);
                    }
                    return;
                ...
                if (m_time_left_rising > 0) {
                    console::log(class_name(), "::collide_y() time left rising > 0\n");
                    return;
                }
            ...
            case Type::clip_ledge_R_50: {
                ...
                if (our_UL.x > other_DR.x - 2.0F or
                    our_DR.x < other_UL.x + 2.0F) {
                    collide_x(our, other);
                    return;
                }
                ...
                if ((our_velocity.y > 0.0F and our_DR.y < other_UL.y + 2.0F) or
                    ...
                    if (our_DR.x + our_velocity.x < other_UL.x + 2.0F or our_UL.x + our_velocity.x > other_DR.x - 2.0F) {
                ...
                if (our_velocity.y > 0.0F and our_DR.y - 2.0F < other_UL.y) {
                    ...
                    velocity_y(0.0F);
                    move_velocity_y(0.0F);
                } else if (our_velocity.y < 0.0F and our_UL.y > other.owner->start_offset().y + 16.0F - camera::position.y) {
                    velocity_y(std::abs(velocity().y) * 0.75F);
            ...
            case Type::frog: {
                ...
                if (our_UL.y < other_UL.y + other_extent.y / 2.0F and our_velocity.y > 0.0F) {
                    position_add_y(-overlap_y);

                    //console::log(class_name(), "::collide_y() frog below\n");
                    if (our_velocity.y >= 6.0F) {
                        other.owner->hurt(this);
                    ...
                    //console::log(class_name(), "::collide_y() frog above\n");
                    //other.owner->velocity_y(our_velocity.y);
                    //velocity_y(other_velocity.y - our_velocity.y);
                }

                //console::log("other vx: ", other_velocity.x, " ", our.owner->move_velocity().x, "\n");        
            ...
            case Type::slope_L_1x1: {
                ...
                //}
                if (m_is_down_thrusting and our_velocity.y > 0.0F) {
                    spawn_down_thrust(position() + Vec2F{ 8.0F, 8.0F },
                                      Vec2F{ -3.0F, 0.0F },
                                      Vec2F{  3.0F, 0.0F });
                }
            ...
            case Type::spring_U: {
                ...
                if (m_state == state::Type::swim) {
                    ...
                    m_is_down_thrusting ? reset_anim("jump_spin") : reset_anim("jump");
                    m_num_jumps = 1;
                    velocity_y(-4.5F - our_velocity.y / 4.5F);
                }
                console::log(class_name(), "::collide_y() spring_U velocity y: ", velocity().y, " ", max_velocity().y, "\n");
                m_time_left_bouncing = m_config.time_to_bounce();
                m_is_down_thrusting = false;
                m_is_on_ground = false;
                m_is_on_slope = false;
            ...
            case Type::water_line: {
                ...
                if (std::abs(velocity().y) > 1.0F) {
                    cF32 vel_x = velocity().x + move_velocity().x;
                ...
                //other.owner->velocity_x(/*other.owner->move_velocity().x * 0.5F + */velocity().y);
                ...
                if (our_velocity.y > 0.0F and m_state != state::Type::swim) {
                    if (our_center.y <= other_UL.y) return;
                    is_upwards = false;
                ...
                } else if (our_velocity.y < 0.0F and m_state == state::Type::swim) {
                    if (our_center.y > other_DR.y) return;
                    if (velocity().y >= -1.5F or is_pressed(key_jump)) {
                    ...
                    if (velocity().y <= -1.0F and !is_pressed(key_jump)) {
                        ...
                        if (m_next_state != state::Type::dive) {
                            ...
                            velocity_y(our_velocity.y * 1.2F);
