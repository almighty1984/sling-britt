...
namespace entity {
    void Player::collide_y(aabb::cInfo our, aabb::cInfo other) {
        ...
        cVec2F our_extent = { our_points.w - our_points.x, our_points.h - our_points.y };
        ...
        //if (other_points.y < our_points.y and velocity().y >= 0.0F or
            //other_points.y > our_points.y and velocity().y <= 0.0F) return;
        ...
        if (our_velocity.y < 0.0F and !is_ducking()) {
            if (m_state != state::Type::swim and our_points.y > other_points.h - 4.0F) {
        ...
        if ((m_state == state::Type::run or m_state == state::Type::shoot) and our_points.y < other_points.y) {
            if (!m_is_on_ground and !m_is_on_slope and velocity().y > 1.0F and m_time_left_ducking == 0 and
                (is_clip(other_type) or
                    is_slope(other_type))
                ) {
                particle::spawn({ .parent = this,
                                  .type = particle::Type::dust,
                                  .position = Vec2F{ our_points.x + 8.0F, other_points.y },
        ...
        if (is_arch(other_type)) {
            ...
            if (our_points.y < other_points.y or m_time_left_ducking > 0) {
                return;
            }
            position_add_y(-overlap_y);
        ...
        if (is_slope(other_type) and other_type != Type::slope_U and m_state == state::Type::swim) {            
            ...
            /*if (velocity().y < 0.0F and our_points.y < other_points.h - 2.0F or velocity().y > 0.0F and our_points.h > other_points.y + 2.0F) {
                collide_x(our, other);
                return;
            }*/

            if (our_points.h < other_points.y + 4.0F and our_velocity.y > 0.0F) {
            ...
            /*if (our_points.h < other_points.y + 2.0F and velocity().y > 0.0F) {
                velocity_y(0.0F);
                m_is_on_ground = true;
            } else if (our_points.y > other_points.y - 2.0F and velocity().y < 0.0F) {
            ...
            if (other.owner->state() == state::Type::swim) {
                if ((m_state == state::Type::swim or m_state == state::Type::dive) and velocity().y < 0.0F and our_points.h > other_points.h) {
                ...
                else if (our_velocity.y > 0.0F and our_points.y < other_points.y) {
            ...
            if (our_points.y + 4.0F >= other_points.y) {   // bug above
                ...
                cF32 our_mid_x = our_points.w - our_points.x / 2.0F;
                cF32 other_mid_x = other_points.w - other_points.x / 2.0F;
            ...
            if (velocity().y > 0.0F and our_points.y < other_points.y) {
                m_is_on_ground = true;
                m_is_on_slope = false;
            ...
            if (m_state == state::Type::swim and our_points.h > other_points.h) {
                velocity_y(0.0F);
                moved_velocity_y(0.0F);
                position_add_y(-overlap_y);
                return;
            }
            if ((our_points.h > other_points.h and m_is_on_ground) or m_is_climbing_ledge) {
                m_time_left_rising = m_time_to_rise;
                m_time_left_ducking = m_time_to_duck;
                return;
            }
            ...
            if (our_velocity.y < 0.0F and our_velocity.y > -10.0F and our_points.y < other_points.h - 4.0F or
                our_velocity.y > 0.0F and our_velocity.y <  10.0F and our_points.h > other_points.y + 4.0F) {
                collide_x(our, other);
                return;
            }
            if ((our_velocity.y > 0.0F and our_points.h > other_points.y + 4.0F) or
                (our_velocity.y < 0.0F and our_points.y < other_points.h - 2.0F)) {
                //collide_x(our, other);
                if (our_points.w < other_points.x + 2.0F or our_points.x > other_points.w - 2.0F) {
                    console::log(class_name(), "::collide_y() clip_ledge skip\n");
                    return;
                }
            }
            /*console::log(" ", our_points.y, " ", our_points.h, "  ", other_points.y, " ", other_points.h, "\n");*/
            position_add_y(-overlap_y);
            if (m_state == state::Type::swim) {                                    
                return;                
            }
            if (our_velocity.y > 0.0F and our_points.h - 2.0F < other_points.y) {
                m_is_on_ground = true;
                m_is_on_slope = false;
                moved_velocity_x(0.0F);
                velocity_y(0.0F);
            ...
            else if (our_velocity.y < 0.0F and our_points.h > other_points.h) {
                velocity_y(std::abs(velocity().y) * 0.75F);
            ...
            if (our_points.y < other_points.y + 4.0F and our_velocity.y > 0.0F) {
                position_add_y(-overlap_y);
            ...
            if (other_state == state::Type::exit) {
                ...
                if (our_points.h > other_points.h) {
            ...
            if (our_velocity.y < 0.0F or other_name == aabb::Name::bone
                 or our_points.h > other_points.h - 4.0F
                //or our_points.h - 4.0F > other_points.y
                ) {
                return;
            }
            ...
            if (std::abs(velocity().y) > 1.0F) {
                ...
                if (other_points.x + other_extent.x / 2.0F + vel_x < our_points.x + our_extent.x / 2.0F - 2.0F) {
                    //other.owner->force_x(velocity().x / 2.0F - std::abs(velocity().y));
                    other.owner->force_x(-1.0F + velocity().x / max_velocity().x);
                }
                else if (other_points.x + other_extent.x / 2.0F + vel_x > our_points.x + our_extent.x / 2.0F + 2.0F) {
                    //other.owner->force_x(velocity().x / 2.0F + std::abs(velocity().y));
                    other.owner->force_x(1.0F + velocity().x / max_velocity().x);
                }
            }
            ...
            if (velocity().y > 0.0F and m_state != state::Type::swim) {
                ...
                //if (our_points.y + 4.0F > other_points.h) return;
            ...
            if (is_to_splash) {
                ...
                cVec2F pos = Vec2F{ our_points.x + 4.0F, other_points.y } + Vec2F{ 0.0F, is_upwards ? -10.0F : 0.0F };
