...
namespace entity {
    void Player::state_run(cF32 dt) {
        ...
        if (!m_is_on_ground and !m_is_carrying and !m_is_down_thrusting) {
            if (is_pressed(key_up) and is_pressed(key_sprint)) {
                release(key_up);
        ...
        //console::log("layer: ", (int)sprite::layer(m_sling_shot_bg_sprite), "\n");

        if (!m_children.empty()) {
            if (m_children.back()->state() == state::Type::swim or
                m_children.back()->state() == state::Type::hurt) {
                m_children.clear();
                m_is_carrying = false;
            }
        }

        if (sum_velocity().x >= 0.0F or sum_velocity().x < -acceleration().x) {
        ...
        //console::log(class_name(), "::state_run() near wall: ", m_is_near_wall_L, " ", m_is_near_wall_R, "\n");
        
        if (m_is_carrying and m_current_anim != anim("jump_spin")) {
            sprite_texture("res/texture/entity/player/britt_carry.png");
        } else {
        ...
        if (is_pressed(key_shoot) and m_time_left_until_down_thrust == 0 and !m_is_down_thrusting and m_state != state::Type::duck) {
            lock(key_shoot);
        ...
        if (is_pressed(input::Key::h)) {
            ...
            console::log(class_name(), "::state_run() heatlh: ", health_amount(), "\n");
        }
        ...
        if (m_time_left_melee > 0) {
            ...
            if (m_time_left_melee == m_time_to_melee) {
                particle::spawn({ .parent = this,
                                  ...
                                  .position = position() + Vec2F{ sprite_is_leftward() ? -8.0F : 14.0F, 0.0F},
                                  .velocity = velocity() + move_velocity() + Vec2F{sprite_is_leftward() ? -2.0F : 2.0F, 0.0F},
        ...
        if (m_time_left_hitting_ground > 0) {
            if (m_time_left_hitting_ground == m_time_to_hit_ground) {
                particle::spawn_fan(this, 180.0F, 0.0F, 8, particle::Type::drop_blood,
                                    position() + Vec2F{ 0.0F, 4.0F }, {}, 5.0F,
                                    state::Type::idle);
                particle::spawn_fan(this, 180.0F, 0.0F, 8, particle::Type::drop_blood,
                                    position() + Vec2F{ 0.0F, 4.0F }, {}, 4.0F,
                                    state::Type::idle);
        ...
        if (m_is_on_ground and m_time_left_rising == 0 and
        ...
        if (m_is_on_ground and m_time_left_melee == 0 and m_time_left_skidding == 0) {
            if (velocity().x >= -acceleration().x and velocity().x <= acceleration().x) {
                ...
                is_pressed(key_sprint) and !m_is_carrying ? set_anim("run") : set_anim("walk");
            }
        }
        ...
        if (!m_is_on_ground and m_time_left_melee == 0) {
        ...
        //console::log(class_name(), "::state_run() is near wall: ", m_is_near_wall_L, " ", m_is_near_wall_R, "\n");
        
        if (is_pressed(key_down) and !is_locked(key_down)) {
            if (m_is_carrying and !m_children.empty()) {
                console::log(class_name(), "::state_run() child near wall: ", m_children.back()->is_near_wall_L(), " ", m_children.back()->is_near_wall_R(), "\n");
                if ( sprite_is_leftward() and !m_children.back()->is_near_wall_L() or 
                    !sprite_is_leftward() and !m_children.back()->is_near_wall_R()
                    ) {
                ...
                console::log(class_name(), "::state_run() down thrust ", m_time_left_rising, "\n");
                release(key_down);
                release(key_jump);
                ...
                sound_position("down_thrust", { (position().x + 8.0F) / app::config::extent().x / 2.0F,
        ...
        if (is_pressed(input::Key::num_5)) {
            ...
            m_time_left_until_next_state = 20;
        }
        ...
        if (m_next_state == state::Type::swim) {
            if (m_time_left_until_next_state > 0) --m_time_left_until_next_state;
        ...
        if (m_next_state == state::Type::sling and m_time_left_until_next_state == 0) {
