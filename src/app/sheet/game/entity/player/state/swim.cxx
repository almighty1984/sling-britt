...
namespace entity {
    void Player::state_swim(cF32 dt) {
        ...
        if (velocity().x != 0.0F and velocity().y != 0.0F) {
            //prev_swim_speed = m_swim_speed;
            m_swim_speed = std::sqrtf(velocity().x * velocity().x + velocity().y * velocity().y);
        }
        else if (velocity().x == 0.0F) {
            m_swim_speed = velocity().y;
        }
        else if (velocity().y == 0.0F) {
            m_swim_speed = velocity().x;
        }
        else {
            //m_swim_speed = prev_swim_speed;
        }
        ...
        deceleration(is_bouncing() ? Vec2F{0.0F, 0.0F} : Vec2F{ 0.02F, 0.02F });


        //console::log(class_name(), "::state_swim() dec: ", deceleration().x, " ", deceleration().y, "\n");

        cF32 swim_speed_limit = is_pressed(key_sprint) ? 4.0F : 2.5F;
        //cF32 swim_speed_limit = 3.0F;
                
        //console::log(class_name(), "::state_swim() speed: ", m_swim_speed, "\n");

        Vec2F swim_acc = { 0.2F, 0.2F };

        // steer slower at high speed
        if (!is_bouncing() and std::abs(m_swim_speed) > swim_speed_limit) {
            swim_acc *= 0.5F;
