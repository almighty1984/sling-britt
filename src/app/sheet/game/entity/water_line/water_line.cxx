...
namespace entity {
    ...
    void WaterLine::update() {
        ...
        if (m_is_first_update) {
            ...
            line::layer(m_bottom_line, m_start_layer);

            m_top_line = line::make(position() + Vec2F{ 0.0F, 2.0F },
                                       m_input_objects.back()->position() + Vec2F{ 0.0F,
                                       m_line_offset_y });
        ...
        line::set(m_bottom_line, start + Vec2F{ 0.0F, size_offset * m_top_line_size },
                                    end + Vec2F{ 0.0F, size_offset * m_top_line_size });


        Vec2F l;
        l.x = line::end(m_top_line).x - line::start(m_top_line).x;
        if (line::end(m_top_line).y > line::start(m_top_line).y) {
            l.y = line::end(m_top_line).y - line::start(m_top_line).y;
        }
        else {
            l.y = line::start(m_top_line).y - line::end(m_top_line).y;
        }
