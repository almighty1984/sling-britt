...
namespace sheet {
    MenuStart::MenuStart(cU16 window_w, cU16 window_h) {
        ...
        m_type = m_transition_to = m_transition_from = Type::menu_start;
        m_input = input::make();

        m_transform = transform::make();
        transform::position(m_transform, { window_w / 2.0F, window_h / 2.0F });
        transform::deceleration(m_transform, { 0.05F, 0.05F });

        m_line = line::make({ 0.0F, 0.0F }, { 32.0F, -16.0F });
        line::size(m_line, 2.0F);
        line::color(m_line, { 52, 206, 206 });
        line::layer(m_line, 0);
        line::transform(m_line, m_transform);
        ...
        line::transform(m_normal_id, m_transform);
        ...
        line::transform(m_proj_on_normal_id, m_transform);
    ...
    void MenuStart::update(cF32 dt) {
        ...
        if (input::mouse_moved) {
            m_is_mouse_on_selection = false;
            if (input::mouse.x >= m_game_text.position().x and
        ...
        //console::log("is_mouse_on_selection: ", m_is_mouse_on_selection, "\n");
        ...
        if (is_pressed(input::Key::f1)) {
            ...
            transition(Type::menu_start, Type::edit);
            return;
        }
        ...
        if (is_pressed(input::Key::f3)) {
            ...
            transition(Type::menu_start, Type::menu_options);
            return;
        }
        ...
        if (is_pressed(input::Key::w)) {
            transform::velocity_add_y(m_transform, -0.1F);
        }
        if (is_pressed(input::Key::s)) {
            transform::velocity_add_y(m_transform, 0.1F);
        }
        if (is_pressed(input::Key::a)) {
            transform::velocity_add_x(m_transform, -0.1F);
        }
        ...
        cVec2F point = m_circle.position() - transform::position(m_transform);
        ...
        if (point_dot_normal <= 0.0F) {
            ...
            //m_circle.velocity_x(-proj_onto_normal.x * 0.2F + transform::velocity(m_transform).x * 1.0F);
            //m_circle.velocity_y(-proj_onto_normal.y * 0.2F + transform::velocity(m_transform).y * 1.0F);

            //m_circle.velocity_add_x(transform::velocity(m_transform).x * 0.1F);
            //m_circle.velocity_add_y(transform::velocity(m_transform).y * 0.1F);
            ...
            if (speed_along_normal <= 0.0F) {
                ...
                //    console::log("state::Menu::update() speed_x: ", speed_x, " speed_y: ", speed_y, "\n");
                    ...
                    m_circle.velocity_x(/*-proj_onto_normal.x * 0.01F +*/ speed_x * 0.01F + transform::velocity(m_transform).x);
                    m_circle.velocity_y(/*-proj_onto_normal.y * 0.01F +*/ speed_y * 0.01F + transform::velocity(m_transform).y);
                //                            
                    //m_circle.velocity_add_x(speed_x);
                    //m_circle.velocity_add_y(speed_y);

                //    m_circle.velocity_add_x(transform::velocity(m_transform).x);
                //    m_circle.velocity_add_y(transform::velocity(m_transform).y);
                //}


                /*if (is_pressed(input::Key::up)) {
                    release(input::Key::up);
                    transform::velocity_add(m_circle.transform(), normal * 0.2F);
                }*/
            }
        ...
        if (is_pressed(input::Key::ctrl)) {
            if (is_pressed(input::Key::up)) {
                transform::velocity_add_y(m_circle.transform(), -move_speed);
            }
            if (is_pressed(input::Key::down)) {
                transform::velocity_add_y(m_circle.transform(), move_speed);
            }
            if (is_pressed(input::Key::left)) {
                transform::velocity_add_x(m_circle.transform(), -move_speed);
            }
        ...
        //        //m_circle.velocity_x(speed_x * 0.01F + transform::velocity(m_transform).x);
        //        //m_circle.velocity_y(speed_y * 0.01F + transform::velocity(m_transform).y);
        ...
        //console::log("mouse moved: ", input::mouse_moved, "\n");
        ...
        if (is_pressed(input::Button::left) and m_is_mouse_on_selection) {
        ...
        if (is_to_change_sheet) {
            ...
            switch (s_selection) {
                case 0: transition(Type::menu_start, Type::game);         break;
                case 1: transition(Type::menu_start, Type::edit);         break;
                case 2: transition(Type::menu_start, Type::menu_options); break;                
            }
