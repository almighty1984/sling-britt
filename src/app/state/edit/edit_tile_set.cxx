...
namespace state {
    bool Edit::load_types_from_text_file(const std::filesystem::path& path) {
        ...
        if (start == std::string::npos) {
            console::error("state::Edit::load_types_from_text_file() set_255 not found\n");
            return false;
        }

        const size_t open_bracket = file_text.find("{", start);
        if (open_bracket == std::string::npos) {
            console::error("state::Edit::load_types_from_text_file() set_255 open bracket not found\n");
            return false;
        }
        const size_t close_bracket = file_text.find("}", open_bracket);
        if (close_bracket == std::string::npos) {
            console::error("state::Edit::load_types_from_text_file() set_255 close bracket not found\n");
            return false;
        }
