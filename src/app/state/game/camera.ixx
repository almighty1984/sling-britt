...
export namespace camera {
    ...
    Vec2I level_scroll_limit = { INT_MAX, INT_MAX };
    ...
    void clear() {
        ...
        level_scroll_limit = { INT_MAX, INT_MAX };
    }
    ...
    void update() {
        ...
        if (transform::position(focus_transform).x + focus_offset.x > focal_point.x ||
            ...
            if (difference.x < 0.0F && position.x <= 0.0F) {
                difference.x = -position.x;
            }
            else if (difference.x > 0.0F && position.x >= level_scroll_limit.x) {
