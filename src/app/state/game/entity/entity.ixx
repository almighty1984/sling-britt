...
export namespace entity {
    ...
    public:
        ...
        State prev_state() { return m_prev_state; }

        U8 num_jumps() const { return m_num_jumps; }   void num_jumps(cU8 n) { m_num_jumps = n; }

        bool is_in_view() { return position().x + sprite_source_rect().w > 0.0F && position().x < WINDOW_W &&
                                   position().y + sprite_source_rect().h > 0.0F && position().y < WINDOW_H; }

        bool is_start_in_view() { return start_position().x + sprite_source_rect().w > 0.0F && start_position().x < WINDOW_W &&
                                         start_position().y + sprite_source_rect().h > 0.0F && start_position().y < WINDOW_H; }
