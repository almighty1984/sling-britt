...
export namespace entity {
    ...
    public:
        ...
        void  direction_x(cF32 x) { m_direction.x = x; }
        void  direction_y(cF32 y) { m_direction.y = y; }
        ...
        bool is_in_view() { return position().x + sprite_source_rect().w > 0.0F and position().x < app::config::extent().x and
                                   position().y + sprite_source_rect().h > 0.0F and position().y < app::config::extent().y; }

        bool is_start_in_view() { return start_position().x + sprite_source_rect().w > 0.0F and start_position().x < app::config::extent().x and
                                         start_position().y + sprite_source_rect().h > 0.0F and start_position().y < app::config::extent().y; }
