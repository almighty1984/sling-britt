...
namespace state {
    void Game::draw(std::unique_ptr<Window>& window, cU8 layer) {        
        ...
        if (sprite::layer(m_player.sprite_id()) == layer) {
            m_is_drawing_debug ? m_player.draw_debug(window) : m_player.draw(window);
        }
        for (auto& i : m_entity_objects) {
            if (i && sprite::layer(i->sprite_id()) == layer) {
                m_is_drawing_debug ? i->draw_debug(window) : i->draw(window);
            }
        }
        ...
        m_is_drawing_debug ? particle::draw_debug(window, layer) : particle::draw(window, layer);

        if (m_is_drawing_debug && layer == 0) {
